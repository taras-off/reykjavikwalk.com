...
PUBLISHED_IDX = [8]          # 8 = «один день в Рейкьявике»
...
def lang_switcher(lang, path="", paths=None):
    ...
    for l in LANGS:
        ...
        p = (paths or {}).get(l, path)
        items.append(f'<a{cls} href="{url(l, p).replace(SITE, "") or "/"}">{LANG_LABEL[l]}</a>')
...
<script>
/* Bókun: движок грузится только после первого действия пользователя (Lighthouse его не делает),
   а клик по кнопке/ссылке всегда открывает модалку — никакой отдельной страницы. */
(function(){{
  ...
  function load(){{
    ...
    s.src='https://widgets.bokun.io/assets/javascripts/apps/build/BokunWidgetsLoader.js?bookingChannelUUID='+UUID;
    document.body.appendChild(s);
  }}
  ['mouseover','touchstart','scroll','keydown','pointerdown'].forEach(function(e){{
    window.addEventListener(e,load,{{once:true,passive:true}});
  }});
  function engineReady(){{
    return typeof window.BokunWidgets==='function' && typeof window.BokunWidgets.openModal==='function';
  }}
  document.addEventListener('click',function(ev){{
    ...
    (function poll(){{
      if(engineReady()){{
        el.classList.remove('is-bokun-loading');
        window.BokunWidgets.openModal({{src:WIDGET+'?isModal=true',openFrom:'bookingButton',buttonId:el.id||''}});
        return;
      }}
      if(Date.now()-t0>8000){{  /* движок не поднялся: не бросаем пользователя, уводим в новую вкладку */
        el.classList.remove('is-bokun-loading');
...
def header(lang, c, path, paths=None):
    L = c["ui"]
    home = "/" if lang == "en" else f"/{lang}/"
    guides_home = home + c["slugs"]["guides"] + "/"
    pub = [c["guides"][i] for i in PUBLISHED_IDX if i < len(c["guides"])]
    drop = "".join(
        f'<a href="{home}{g["slug"]}/">{g["title"]}</a>' for g in pub)
...
def footer(lang, c):
    ...
    explore = "".join(f'<li><a href="{home}{g["slug"]}/">{g["title"]}</a></li>'
                      for g in (c["guides"][i] for i in PUBLISHED_IDX if i < len(c["guides"])))
    return f'''<footer>
  <div class="wrap">
    <div class="grid g4">
      <div>
...
def guides_page(lang, c):
    ...
    blocks = []
    pub = {id(c["guides"][i]) for i in PUBLISHED_IDX if i < len(c["guides"])}

    def card(g):
        if id(g) in pub:
            return (f'<a class="card gcard" href="{home}{g["slug"]}/"><div class="pad">'
                    f'<h3>{g["title"]}</h3><p class="meta">{g["desc"]}</p></div></a>')
        return (f'<div class="card gcard"><div class="pad"><h3>{g["title"]}</h3>'
                f'<p class="meta">{g["desc"]}</p><span class="soon">{L["in_production"]}</span></div></div>')

    for w in ("P0", "P1", "P2"):
        cards = "".join(card(g) for g in waves.get(w, []))
...
def sitemap(contents):
    ...
    def entry(path_by_lang, prio, freq="monthly", lastmod="2026-08-06"):
